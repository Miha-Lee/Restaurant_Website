...
(function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    _init: function () {
      ...
      self.$element.find(".pogoSlider-slide").each(function () {
        ...
        $(this)
          ...
          .each(function () {
            var startTime =
              parseInt($(this).data("start")) !== undefined
                ? $(this).data("start")
                : self.settings.elementTransitionStart;
            var duration =
              parseInt($(this).data("duration")) ||
              self.settings.elementTransitionDuration;
            ...
            children.push({
              ...
              duration: duration,
              transitionIn:
                $(this).data("in") || self.settings.elementTransitionIn,
              transitionOut:
                $(this).data("out") || self.settings.elementTransitionOut,
            });
        ...
        var slide = {
          ...
          element: this,
          transition:
            $(this).data("transition") || self.settings.slideTransition,
          duration:
            parseInt($(this).data("duration")) ||
            self.settings.slideTransitionDuration,
          elementTransitionDuration: elementTransitionDuration,
          totalSlideDuration:
            self.settings.autoplayTimeout + elementTransitionDuration,
          children: children,
        };
      ...
      self.$element.css(
        "padding-bottom",
        100 / (self.settings.targetWidth / self.settings.targetHeight) + "%"
      );
    ...
    _createNavigation: function () {
      ...
      var $navContainer = $('<ul class="pogoSlider-nav"></ul>').appendTo(
        self.$element
      );
    ...
    getAppliedProps: function (el) {
      ...
      for (var sheet in styleSheets) {
        if (styleSheets.hasOwnProperty(sheet)) {
          var cssRules =
            styleSheets[sheet].rules || styleSheets[sheet].cssRules;
          for (var r in cssRules) {
            if (el.matches(cssRules[r].selectorText)) {
              var matchedStyles = stylesReg.exec(
                cssRules[r].cssText.replace(/ /g, "")
              );
              if (matchedStyles) {
        ...
        for (var i = 0; i < appliedProps.length; i++) {
          ...
          if (unitReg.test(cssVal) && numReg.test(cssVal)) {
            ...
            if (pixelReg.test(unitVal[0])) {
              styleObj[appliedProps[i]] =
                Math.ceil(numVal[0] * scaleFactor) + unitVal[0];
            } else {
    ...
    pause: function () {
      if (this.settings.autoplay) {
        ...
        for (
          var i = 0;
          i < this.slides[this.currentSlideIndex].children.length;
          i++
        ) {
          this.slides[this.currentSlideIndex].children[i].$element.precss(
    ...
    _changeSlide: function (prevSlideIndex, currentSlideIndex) {
      ...
      var slideTransition = slideTransitions[
        self.slides[currentSlideIndex].transition
      ]
        ? self.slides[currentSlideIndex].transition
        : "slide";
      var slideTransitionCallback = slideTransitions[slideTransition].apply(
        self,
    ...
    _onSlideStart: function (slideIndex) {
      ...
      if (this.paused) {
        for (var i = 0; i < this.slides[slideIndex].children.length; i++) {
          this.slides[slideIndex].children[i].$element.precss(
            "animation-play-state",
            "paused"
          );
        }
    ...
    _onSlideEnd: function (slideIndex) {
      ...
      if (this.paused) {
        timeElapsed =
          this.slides[slideIndex].totalSlideDuration - this.slideTimeRemaining;
        ...
        timeElapsed =
          this.slides[slideIndex].totalSlideDuration -
          (this.slideTimeRemaining - (new Date() - this.slideStartTime));
      }

      // transition out, if slides elements have already transitioned in
      if (
        this.slides[slideIndex].children.length > 0 &&
        timeElapsed > this.slides[slideIndex].elementTransitionDuration
      ) {
    ...
    _slideCleanup: function (slideIndex, slideVisible) {
      if (
        this.slides[slideIndex].$element.find(".pogoSlider-slide-slice")
          .length > 0
      ) {
      ...
      this.slides[slideIndex].$element
        .attr("style", this.slides[slideIndex].$element.data("original-styles"))
        .css("opacity", slideVisible ? "1" : "0");
    },
    ...
    _slideElementCleanup: function (slideIndex) {
      ...
      for (var i = 0; i < this.slides[slideIndex].children.length; i++) {
        this.slides[slideIndex].children[i].$element
          .removeClass(removePogoSlideElementClasses)
          .attr("style", removePogoSlideElementStyles)
          .css("opacity", 0);
      }
    },
    ...
    _createSlideSlices: function (slideIndex, rows, cols) {
      ...
      if (
        slideIndex === this.prevSlideIndex &&
        this.slides[slideIndex].children.length > 0 &&
        timeElapsed < this.slides[slideIndex].elementTransitionDuration
      ) {
        for (var i = 0; i < this.slides[slideIndex].children.length; i++) {
          var animationDelay =
            this.slides[slideIndex].children[i].startTime - timeElapsed + "ms";
      ...
      for (var j = 0; j < numSlices; j++) {
        ...
        el.attr("style", slicePosStyles)
          ...
          .attr(
            "style",
            styleAttr + sliceInnerPosStyles + sliceInnerBackgroundStyles
          );
      }
    },
    ...
    slideTransitions: {
      ...
      slide: function (prevSlideIndex, currentSlideIndex) {
        ...
        return this.slideTransitions[method].apply(this, [
          prevSlideIndex,
          currentSlideIndex,
        ]);
      },
      ...
      slideLeft: function (prevSlideIndex, currentSlideIndex) {
        ...
        return function () {
          self.slides[prevSlideIndex].$element.removeClass(
            "pogoSlider-animation-leftOut"
          );
          currentSlide.$element
            .attr("style", currentSlide.$element.data("original-styles"))
            .css("opacity", "1")
      ...
      verticalSplitReveal: function (prevSlideIndex, currentSlideIndex) {
        ...
        self.slides[prevSlideIndex].$element.css(
          "z-index",
          self.settings.baseZindex + 1
        );
        currentSlide.$element.css({
          opacity: 1,
          "z-index": self.settings.baseZindex,
        ...
        var $slices = self.slides[prevSlideIndex].$element.find(
      ...
      zipReveal: function (prevSlideIndex, currentSlideIndex) {
        ...
        self._createSlideSlices(
          prevSlideIndex,
          1,
          Math.round(self.$element.width() / 100)
        );
      ...
      barRevealDown: function (prevSlideIndex, currentSlideIndex) {
        return this.slideTransitions["barReveal"].apply(this, [
          prevSlideIndex,
          currentSlideIndex,
          "down",
        ]);
      ...
      foldRight: function (prevSlideIndex, currentSlideIndex) {
        ...
        self._createSlideSlices(prevSlideIndex, 1, 2);
        var $prevSlideSlices = prevSlide.$element.find(
          ".pogoSlider-slide-slice"
        );

        self._createSlideSlices(currentSlideIndex, 1, 2);
        var $currentSlideSlices = self.slides[currentSlideIndex].$element.find(
          ".pogoSlider-slide-slice"
        );
    ...
    compatSlideTransitions: {
      ...
      slideLeft: function (prevSlideIndex, currentSlideIndex) {
        ...
        currentSlide.$element
          ...
          .animate({ left: 0 }, currentSlide.duration);
      },
      ...
      slideUp: function (prevSlideIndex, currentSlideIndex) {
        ...
        currentSlide.$element
          ...
          .animate({ top: "0" }, currentSlide.duration);
      },
      ...
      slideRevealLeft: function (prevSlideIndex, currentSlideIndex) {
        ...
        this.slides[prevSlideIndex].$element
          ...
          .animate({ left: "-100%" }, currentSlide.duration);
      ...
      slideOverRight: function (prevSlideIndex, currentSlideIndex) {
        ...
        currentSlide.$element
          ...
          .animate({ right: 0 }, currentSlide.duration);
      },
        ...
        $slices.each(function (index) {
          if (index % 2 === 0) {
            $(this)
              ...
              .animate({ top: "100%" }, transitionDuration);
          } else {
            $(this)
              .delay(transitionDelay * index)
              .animate({ top: "-100%" }, transitionDuration);
